...
import { getBinding, clearBinding, restoreBinding, autoDetectAndBind, bindToWindow, fetchWindows, getBindingArgs, } from "./binding.js";
...
const RpToolSchema = Type.Object({
    ...
    confirmEdits: Type.Optional(Type.Boolean({ description: "Confirm edit-like operations (required when confirmEdits is enabled)" })),
    // Formatting
...
// ─────────────────────────────────────────────────────────────────────────────
export default function repopromptMcp(pi) {
            ...
            switch (subcommand) {
                ...
                case "bind": {
                    ...
                    try {
                        ...
                        ctx.ui.notify(`Bound to window ${binding.windowId}` +
                            (binding.workspace ? ` (${binding.workspace})` : "") +
                            (binding.tab ? `, tab "${binding.tab}"` : ""), "info");
                    }
                    catch (err) {
                ...
                case "reconnect":
                    ...
                    break;
                default:
                    ctx.ui.notify("RepoPrompt commands:\n" +
                        ...
                        "  /rp reconnect    - Reconnect to RepoPrompt", "info");
            }
        },
    });
    // ───────────────────────────────────────────────────────────────────────────
...
Mode priority: call > describe > search > windows > bind > status`,
        ...
        renderResult(result, options, theme) {
            ...
            // Collapsed view
            if (!options.expanded) {
                const { content, truncated, totalLines } = prepareCollapsedView(textContent, theme, config.collapsedMaxLines);
                if (truncated) {
    ...
    function parseSelectionSummaryFromJson(value) {
        if (!value || typeof value !== "object") {
            return null;
        }
        const root = value;
        const selection = root.selection && typeof root.selection === "object"
            ? root.selection
            : null;
        const summary = root.summary && typeof root.summary === "object" ? root.summary : null;
        const candidates = [root, selection, summary].filter(Boolean);
    ...
    function parseSelectionSummaryFromText(text) {
    ...
    async function showStatus(ctx) {
        ...
        if (binding) {
            ...
            if (binding.workspace)
                msg += `  Workspace: ${binding.workspace}\n`;
            if (binding.tab)
                msg += `  Tab: ${binding.tab}\n`;
            if (binding.autoDetected)
                msg += `  (auto-detected from cwd)\n`;
    ...
    async function showWindows(ctx) {
        const windows = await fetchWindows();
        if (windows.length === 0) {
            ctx.ui.notify("No RepoPrompt windows found", "warning");
            return;
    ...
    async function executeListWindows() {
        ...
        if (windows.length === 0) {
            return {
                content: [{ type: "text", text: "No RepoPrompt windows found. Is RepoPrompt running?" }],
    ...
    async function executeBinding(extensionApi, windowId, tab) {
        const binding = await bindToWindow(extensionApi, windowId, tab, config);
        let text = `## Bound ✅\n`;
        text += `- **Window**: ${binding.windowId}\n`;
        if (binding.workspace)
            text += `- **Workspace**: ${binding.workspace}\n`;
        if (binding.tab)
            text += `- **Tab**: ${binding.tab}\n`;
        return {
            content: [{ type: "text", text }],
    ...
    async function executeDescribe(toolName) {
        const client = getRpClient();
        const normalized = normalizeToolName(toolName);
        const tool = client.tools.find((t) => t.name === toolName || t.name === normalized || normalizeToolName(t.name) === normalized);
        if (!tool) {
            return {
                content: [{ type: "text", text: `Tool "${toolName}" not found. Use rp({ search: "..." }) to search.` }],
    ...
    async function executeToolCall(params, onUpdate) {
        const client = getRpClient();
        const toolName = normalizeToolName(params.call);
        // Validate tool exists
        const tool = client.tools.find((t) => t.name === toolName || normalizeToolName(t.name) === toolName);
        if (!tool) {
            return {
                content: [{ type: "text", text: `Tool "${params.call}" not found. Use rp({ search: "..." }) to search.` }],
        ...
        if (!guardResult.allowed) {
            return {
                content: [{ type: "text", text: guardResult.reason }],
        ...
        try {
            ...
            let responseContent = content.length > 0 ? content : [{ type: "text", text: "(empty result)" }];
            if (editNoop && !result.isError) {
                responseContent = [
                    { type: "text", text: "⚠ No changes applied (no-op edit)" },
                    ...responseContent,
                ];
            }
            return {
...
// Initialization
// ─────────────────────────────────────────────────────────────────────────────
async function promptForWindowSelection(ctx, candidates) {
    ...
    return await ctx.ui.custom((tui, theme, _kb, done) => {
        ...
        return {
            render(width) {
                ...
                const lines = [];
                const header = theme.fg("accent", theme.bold("RepoPrompt")) +
...
async function initializeExtension(pi, ctx, config) {
        ...
        try {
            ...
            if (windows.length > 0) {
                ...
                if (!stillExists) {
                    clearBinding();
                    if (ctx.hasUI) {
                        ctx.ui.notify(`RepoPrompt: restored binding to window ${restoredBinding.windowId} no longer exists; staying unbound`, "warning");
                    }
                }
            }
        }
    ...
    if (config.autoBindOnStart && !getBinding()) {
        try {
            ...
            if (binding && ctx.hasUI) {
                ctx.ui.notify(`RepoPrompt: auto-bound to window ${binding.windowId} (${binding.workspace ?? "unknown"})`, "info");
            }
                ...
                if (selected) {
                    ...
                    ctx.ui.notify(`RepoPrompt: bound to window ${chosenBinding.windowId} (${chosenBinding.workspace ?? "unknown"})`, "info");
                }
                else {
                    const candidatesText = ambiguity.candidates
                        .map((w) => `${w.id}: ${w.workspace}`)
                        .join(", ");
                    ctx.ui.notify(`RepoPrompt: multiple matching windows for cwd (${candidatesText}). Use /rp bind <id> to choose.`, "warning");
                }
            }
            else if (windows.length > 0 && ctx.hasUI) {
                ctx.ui.notify(`RepoPrompt: ${windows.length} window(s) available. Use /rp bind <id> or rp({ windows: true })`, "info");
            }
        }
        catch (err) {
