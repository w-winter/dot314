 ...
 */
export function renderDiffBlock(code, theme) {
 ...
 */
function isCodemapContent(text) {
...
function renderCodemapBlock(code, theme) {
    const lines = code.split("\n");
    const result = [];
    ...
    for (const line of lines) {
        ...
        // Bullet items: "- something"
            ...
            if (content.match(/^[\w-]+$/) && !inCodeSection) {
                ...
                result.push(...highlightedLines.map((line, idx) => (idx === 0 ? firstPrefix : nextPrefix) + line));
            }
        }
 ...
 */
export function renderRpOutput(text, theme, options = {}) {
 ...
 */
function renderMarkdownText(text, theme) {
    const lines = text.split("\n");
    const result = [];
    for (const line of lines) {
 ...
 */
export function prepareCollapsedView(text, theme, maxLines = DEFAULT_COLLAPSED_MAX_LINES) {
    const lines = text.split("\n");
