 ...
 */
export function persistBinding(pi, binding, config) {
 ...
 */
export function restoreBinding(ctx, config) {
    if (config.persistBinding === false) {
        return currentBinding;
    }
...
export function parseWindowList(text) {
    ...
    const windowRegex = /Window\s+`?(\d+)`?\s*•\s*WS:\s*(.+?)\s*•\s*Roots:\s*(\d+)(?:\s*•\s*instance=(\d+))?/gi;
    let match;
...
function parseWindowListFromJson(value) {
    if (!value) {
        return null;
    }
    const windowsValue = Array.isArray(value) ? value : value.windows;
    if (!Array.isArray(windowsValue)) {
        return null;
    }
    const parseIntMaybe = (raw) => {
    ...
    for (const item of windowsValue) {
        ...
        const roots = Array.isArray(obj.roots)
            ? obj.roots.filter((r) => typeof r === "string")
            : [];
...
async function fetchWindowsViaMcp(client) {
...
async function fetchWindowsViaCli() {
    try {
        const { stdout, stderr } = await execFileAsync("rp-cli", ["-e", "windows"], { timeout: 5000, maxBuffer: 1024 * 1024 });
        const output = `${stdout}\n${stderr}`.trim();
...
function normalizeRootLine(line) {
...
export function parseRootList(text) {
    const roots = new Set();
 ...
 */
export async function fetchWindowRoots(windowId) {
    const client = getRpClient();
    if (!client.isConnected) {
        throw new Error("Not connected to RepoPrompt");
    }
 ...
 */
function isPathWithinRoot(dir, root) {
 ...
 */
export function findMatchingWindow(windows, cwd) {
 ...
 */
export async function autoDetectAndBind(pi, config) {
 ...
 */
export async function bindToWindow(pi, windowId, tab, config) {
    const windows = await fetchWindows();
 ...
 */
export function getBindingArgs() {
    if (!currentBinding) {
        return {};
    }
    const args = {
