 ...
 */
export function isDeleteOperation(toolName, args) {
 ...
 */
export function describeDeleteTarget(toolName, args) {
 ...
 */
export function isEditOperation(toolName) {
    const normalizedName = toolName.toLowerCase();
    return (normalizedName === "apply_edits" ||
 ...
 */
export function isNoopEdit(output) {
    const lower = output.toLowerCase();
    // Explicit indicators
    if (lower.includes("search block not found"))
        return true;
    if (lower.includes("no changes"))
        return true;
    if (lower.includes("0 edits applied"))
        return true;
 ...
 */
export function isWorkspaceSwitchInPlace(toolName, args) {
    const normalizedName = toolName.toLowerCase();
 ...
 */
export function checkGuards(toolName, args, config, overrides = {}) {
    // Delete guard
    ...
    if (config.confirmEdits && isEditOperation(toolName) && !overrides.confirmEdits) {
        return {
            allowed: false,
            reason: "Edit confirmation enabled. Re-run with confirmEdits: true to apply this edit (or disable confirmEdits).",
        };
    }
    return { allowed: true };
}
