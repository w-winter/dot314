 ...
 */
function findRepoPromptInMcpConfig() {
...
export function loadConfig(overrides) {
    ...
    const fileConfig = tryReadJson(configPath);
    if (fileConfig) {
        config = { ...config, ...fileConfig };
        const fileConfigAny = fileConfig;
...
function maybeWrapServerCommand(config, server) {
 ...
 */
export function getServerCommand(config) {
    if (config.command) {
    ...
    throw new Error("RepoPrompt MCP server not found. Please ensure rp-mcp-server is installed, " +
