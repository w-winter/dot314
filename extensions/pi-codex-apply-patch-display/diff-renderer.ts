...
const UNNUMBERED_HUNK_HEADER_PATTERN = /^@@(?:\s|$)/;
const COMPACT_FILE_HEADER_PATTERN = /^(?:created|deleted|edited) file\s+(.+)$/;
const MOVED_FILE_HEADER_PATTERN = /^moved file\s+(.+?)\s+→\s+(.+)$/;
...
function classifyMetaLine(raw: string): DiffMetaEntry["kind"] {
  if (
    ...
    || COMPACT_FILE_HEADER_PATTERN.test(raw) || MOVED_FILE_HEADER_PATTERN.test(raw)
  ) {
    return "file";
  }
...
function inferDiffLanguage(rawLine: string): string | undefined {
  ...
  const compactFileHeader = rawLine.match(COMPACT_FILE_HEADER_PATTERN);
  if (compactFileHeader) {
    const filePath = normalizeDiffPath(compactFileHeader[1]);
    return filePath ? detectLanguageFromPath(filePath) : undefined;
  }

  const movedFileHeader = rawLine.match(MOVED_FILE_HEADER_PATTERN);
  if (movedFileHeader) {
    const filePath = normalizeDiffPath(movedFileHeader[2]);
    return filePath ? detectLanguageFromPath(filePath) : undefined;
  }
...
function parseDiff(diffText: string, fallbackFilePath?: string): ParsedDiff {
  ...
  for (const rawLine of diffText.replace(/\r/g, "").split("\n")) {
    ...
    if (UNNUMBERED_HUNK_HEADER_PATTERN.test(rawLine)) {
      hunkIndex += 1;
      stats.hunks = Math.max(stats.hunks, hunkIndex);
      oldLineCursor = null;
      newLineCursor = null;
    ...
    const compactFileHeader = COMPACT_FILE_HEADER_PATTERN.test(rawLine) || MOVED_FILE_HEADER_PATTERN.test(rawLine);
    if (rawLine.startsWith("diff --git ") || rawLine.startsWith("--- ") || rawLine.startsWith("+++ ") || compactFileHeader) {
      if (rawLine.startsWith("diff --git ") || compactFileHeader) {
        stats.files += 1;
      }
